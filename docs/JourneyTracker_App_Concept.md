...
## Notifications v1 — Phase 0: the notification foundation (epic KAN-26; KAN-32 — Decided, not yet built)

**Decided (KAN-32).** Notifications v1 ships in phases; Phase 0 is the plumbing every later phase sits on, with **no user-visible notifications** (a dev/test debug trigger is the only firing in Phase 0). Phase 1 (KAN-33) fires the actual milestone notifications; Phase 2 (KAN-39) adds artwork; v2 (KAN-34) adds settings/reminders/Watch. This section owns the behavior/data/architecture of the foundation and is written so Phase 1 is a trivial content-and-call layer on top.

**Local-only. No push, no APNs, no `aps-environment` entitlement.** Everything is `UNUserNotificationCenter` local scheduling. Nothing here talks to a server (consistent with "What NOT to worry about yet").

**`NotificationManager` — the single authority, mirroring the HealthKitManager grain.** One `@Observable @MainActor final class NotificationManager` with a `static let shared`, the *only* type in the app that touches `UNUserNotificationCenter` — exactly as `HealthKitManager` is the only type that touches HealthKit and `DistanceFormatter`/`StatFormatter` are the only formatting authorities. It owns: category/identifier registration, honest observable authorization state, the contextual permission request, and the fire/schedule primitive. No other file imports `UserNotifications`.

**The payload/identifier assembly lives in exactly one place.** A `MilestoneNotificationRequest` Sendable value type owns the derivation of the request identifier, thread identifier, category identifier, and `userInfo` dictionary from its fields — the one place a notification's on-the-wire shape is assembled, mirroring "formatting happens in exactly one place." Phase 1 supplies only the hook, the ids, and the already-resolved title/body; it never hand-builds an identifier or a `userInfo`.

### Six rulings (KAN-32, Jake)

1. **Contextual permission is requested from the start-journey UI, gated on `authorization == .notDetermined` — not by counting instances.** The request fires in `AvailableJourneysView.start()` immediately after a *successful* `ProgressStore.startJourney`, guarded by `if NotificationManager.shared.authorization == .notDetermined`. This is how "first journey ever" is determined **without a model change**: because the app requests authorization *nowhere else*, the first successful start is the only start where status is still `.notDetermined` — every later start (and every restart, which requires a pre-existing instance) sees a determined status and no-ops. Instance-counting was rejected: it is fragile across KAN-13 delete/re-seed and restart, and the `.notDetermined` gate is self-correcting (deny once → never re-prompt; the OS also coalesces a second request while a prompt is pending). Never requested cold at launch. The request options are standard `[.alert, .sound, .badge]` — not provisional/quiet, because the intent is a real contextual prompt at a moment the user just expressed intent. **As of KAN-42 (below), this system request is fronted by our own pre-permission priming screen** — the priming's "Enable" action is what calls `requestAuthorizationOnFirstJourney()`, and a "Not now" dismissal never calls it, so the OS one-shot is protected. `NotificationManager` remains the single authority; the priming screen only calls this existing method or nothing.

2. **The fire primitive is a synchronous, non-blocking, `nonisolated` enqueue callable straight from the `ProgressStore` `@ModelActor` — it never awaits inside the delta transaction.** Phase 1 detects crossings/completion inside `ProgressUpdater.apply` (KAN-14 Ruling 2), on the actor's one context. It builds `[MilestoneNotificationRequest]` from the crossing **snapshots** (all Sendable value types — UUIDs and resolved strings) *inside* the actor where the models are valid, then calls `NotificationManager.shared.enqueue(_:)`, which returns immediately and hands the work to a detached `Task` that calls `UNUserNotificationCenter.add`. Only Sendable values cross the boundary — never a `@Model`, never the `ModelContext` — mirroring the existing "PersistentIdentifier is Sendable, models aren't" pattern. `enqueue` must not be `@MainActor`-isolated (that would force the actor to hop/await); it is `nonisolated` and self-contained. **This keeps the delta transaction's timing and the actor's serialization exactly as KAN-10/14 left them** — enqueue, don't await UI.

3. **Not authorized ⇒ a first-class silent no-op, and notifications requested before authorization resolves are DROPPED, never queued.** Inside `enqueue`'s detached task, before adding anything, it reads the **live** system authorization from `center.notificationSettings()` (not the cached `@MainActor` property — correct off-main, no staleness) and, unless the status can post (`.authorized`/`.provisional`/`.ephemeral`), it silently returns. Nothing is persisted for later. A milestone is a moment-in-time nudge; replaying a backlog of past-tense "you reached X" hours or days after a late grant would be spam and dishonest — so there is **never a queued flood on a later grant**. This mirrors the KAN-14 crossings' forward-only philosophy: from grant forward, period. Denied/undetermined is a normal, tested code path, never a crash.

4. **`docs/notifications/*.csv` parsing and placeholder fill belong to Phase 1, not Phase 0.** Phase 0's primitive takes an **already-resolved** `title`/`body` (plain `String`s). Rationale: Phase 0 ships no copy and does no firing, so a CSV consumer would have nothing to consume; and the content seam (per-journey lookup, the `{character}`/`{miles_to_next}` placeholder set, `DistanceFormatter` fill at fire time per the notifications README) is exactly what Phase 1 needs to own end-to-end. Building it in Phase 0 would prematurely freeze that seam. Phase 1 adds a `NotificationContentProvider` (CSV parse + placeholder fill via `DistanceFormatter`/`StatFormatter`) that produces the resolved strings the primitive already accepts.

5. **The `userInfo` deep-link schema carries stable `UUID` strings — never `PersistentIdentifier`.** `PersistentIdentifier` is not plist/string-friendly and is not stable across store migrations or devices, so it must not cross the notification boundary. Every model already carries a stable `UUID id` (`UserJourney`, `JourneyTemplate`, `Waypoint`), and `WaypointCrossing` already snapshots `waypointID: UUID` — so UUID strings are the boundary identifier. The stable schema (v1):

   | Key | Type | Meaning |
   |---|---|---|
   | `schemaVersion` | `Int` (= `1`) | payload-shape version, so Phase 2+ can evolve it |
   | `hook` | `String` | `"waypoint_reached"` \| `"journey_complete"` (matches the CSV `hook` column) |
   | `userJourneyID` | `String` (UUID) | the specific run to deep-link to |
   | `templateID` | `String` (UUID) | the catalog template — lets a tap still resolve/route if the instance was deleted |
   | `waypointID` | `String` (UUID), optional | present for `waypoint_reached`; absent for `journey_complete` |

   Phase 1's tap handler resolves `userJourneyID` → `UserJourney` by `UUID` (fetch on `id`), falling back to `templateID`; `waypointID` scrolls the map to that waypoint. Phase 0 defines the schema and the `userInfo` builder; it does not build the tap handler (no UI).

6. **Category, request, and thread identifier scheme (the Phase-1 dedup/threading contract).** All under an app-owned `com.journeytracker.*` namespace (our own app name — no real-world IP):
   - **Categories** (`UNNotificationCategory`, the home for future actions/grouping) — one per milestone type:
     `com.journeytracker.category.waypointReached`, `com.journeytracker.category.journeyComplete`. Registered once at launch (no prompt).
   - **Request identifiers** (`UNNotificationRequest.identifier`, the idempotency/dedup key) — deterministic from the run + milestone, so re-adding the same milestone **replaces** rather than duplicates:
     waypoint → `waypoint.<userJourneyUUID>.<waypointUUID>`; complete → `complete.<userJourneyUUID>`. Combined with KAN-14's crossing idempotency guard, a re-applied delta never double-notifies.
   - **Thread identifiers** (`UNMutableNotificationContent.threadIdentifier`, OS grouping in Notification Center) — **per run**: `journey.<userJourneyUUID>`, so all of a journey's milestones stack together. This is what makes Phase 1's *batching* (several crossings from one delta) a pure content decision — fire several sharing the thread id, or coalesce into one grouped notification — with no plumbing change.

**Where Phase 0 wires in.** `NotificationManager.registerCategories()` + `refreshAuthorizationStatus()` are called once at launch alongside `HealthKitManager.start()` (in `JourneyTrackerApp`/`RootView`), and the manager is injected into the environment like `HealthKitManager`. The contextual request hooks into `AvailableJourneysView.start()` (Ruling 1). A dev-only trigger in `DebugView` fires a sample milestone through the real primitive so the plumbing is verifiable without walking. **This dev trigger uses a short non-repeating time-interval trigger (~5 s) purely so a human tester can background the app and watch the banner land and persist in Notification Center; the real `enqueue(_:)` path Phase 1 uses stays immediate (`trigger: nil`) and unchanged** (KAN-32 QA ruling, 2026-07-14). Foreground suppression is expected: Phase 0 ships **no `UNUserNotificationCenterDelegate`**, so an immediate notification fired while the app is foreground-active is delivered-but-not-presented by iOS. This does not affect the real feature — Phase 1 milestones fire from `ProgressUpdater` during **background** HealthKit delivery, where iOS presents by default without a delegate. **Phase 0 does not touch `ProgressUpdater`/`ProgressStore` or any `@Model`** — the fire call is wired into `ProgressUpdater` in Phase 1; the actor's serialization and the CloudKit schema are untouched (no V4, no migration).

**Out of scope for Phase 0 (KAN-32):** milestone copy and actual firing (Phase 1, KAN-33); CSV parsing/placeholder fill (Phase 1); the `UNUserNotificationCenterDelegate` — both its `willPresent` foreground-presentation policy *and* its `didReceive` tap-to-map deep-link handler (Phase 1, KAN-33). The foreground-presentation policy is a Phase-1 product decision, not a Phase-0 plumbing gap: because real milestones arrive backgrounded, the open question is whether to present a banner at all when the user is *actively in the app* (the map already shows the crossing live) — a likely suppress-or-quiet treatment; any in-app visual belongs to Jeff. Also out of scope: notification artwork (Phase 2, KAN-39); settings, reminders, and Watch delivery (v2, KAN-34); any new UI screen; any SwiftData model or migration change.

### Pre-permission priming screen (KAN-42 — Decided, not yet built)

**Decided (KAN-42).** Before the KAN-32 Ruling-1 system prompt fires, the app shows its **own** screen explaining what notifications it will send and why — the standard priming / pre-permission pattern. This refines, not replaces, KAN-32 Ruling 1: the contextual moment (right after the first successful `startJourney`, gated on `authorization == .notDetermined`) is unchanged; the priming screen simply *fronts* the system request so the one-shot OS prompt is only spent on a user who has already said yes to ours. New UI (visual direction is Jeff's); no new model, no migration, no persistence.

1. **The priming screen intercepts before the system API; `NotificationManager` stays the single authority.** `AvailableJourneysView.start()` no longer calls `requestAuthorizationOnFirstJourney()` inline. Instead, after a successful `startJourney`, if `NotificationManager.shared.authorization == .notDetermined` it presents the priming screen (a modal over the store page, before the store pops back). The priming's **"Enable"** action calls the *existing* `requestAuthorizationOnFirstJourney()` — the only path that ever touches `UNUserNotificationCenter` — then tears down and pops. Any other exit ("Not now" button, or swipe-dismiss if a sheet) calls **nothing**, so `authorization` stays `.notDetermined` and the OS one-shot is unspent and re-offerable. The screen itself imports no `UserNotifications` and holds no authorization logic.

2. **Re-offer policy: gated purely on `.notDetermined`, no persistence, no new state.** The priming shows again on the *next* successful journey start while still `.notDetermined`, mirroring KAN-32's self-correcting gate — Enable → the system prompt resolves the status → never shown again; deny in the system prompt → determined → never shown again. This is **not** nagging: it appears only at a deliberate, user-initiated start moment (never at launch, never repeatedly within a session), and the practical ceiling is naturally bounded by the small catalog plus the one-active-per-template invariant (once a template is active/paused it is no longer startable). No `UserDefaults` "last shown / dismissed count" and **no SwiftData model** are needed for v1. If field use later shows the per-start re-offer is too frequent, a single `UserDefaults` cap can be added with no model/migration change — explicitly deferred, not built now.

3. **Ordering: the run is already committed, so declining never undoes it.** KAN-11 starts the run first; the priming (and the optional system prompt) come after. Because the active `UserJourney` is inserted and saved by `startJourney` *before* the priming appears, a "Not now" leaves a live, accruing journey — exactly as a denied system prompt would under KAN-32. The priming gates only the *ask*, never the *start*.

4. **Presentation is a modal over the store page, not a push.** The priming is presented from `AvailableJourneysView` (still on screen after a successful start, before its pop-back), not pushed onto the `NavigationStack`. Push is rejected: the store already wants to pop-back-on-start, so a pushed priming would fight that flow and a back-swipe would be an ambiguous soft-dismiss. On the priming's completion (either exit) the modal tears down and the store pops back to "Your Journeys," where the new active card is already present via `@Query`. **Whether the modal is a `.sheet` or a `.fullScreenCover` is a visual decision left to Jeff's variants**; a swipe-dismissable sheet's swipe counts as "Not now" (no request), which is correct either way, and an explicit "Not now" control is required regardless.

5. **Hard constraint: must not impersonate the iOS system alert.** No fake two-button "Allow / Don't Allow" OS-styled dialog, no iOS-alert chrome. Our own design-token / `JourneyTheme` styling, §07 button rules, character name via the `JourneyCharacter` seam. The real OS alert is what appears *after* "Enable."

**Out of scope for KAN-42:** the system-prompt mechanics themselves (KAN-32, unchanged); a Settings deep-link for users who denied (that is a determined state the priming never shows in); the full re-engagement cadence / reminders (v2, KAN-34 / KAN-38 territory); any SwiftData model, migration, or persisted counter.

## Notifications v1 — Phase 1: milestone notifications (epic KAN-26; KAN-33 — Decided, not yet built)

**Decided (KAN-33).** Phase 1 fires the real milestone notifications on top of the KAN-32 foundation — a well-crafted, information-rich local notification when a run reaches a waypoint or completes a journey. It is a content-and-call layer over Phase 0's plumbing: **no new detection logic, no new HealthKit path, no schema/migration change, no `@Model` change.** The bar is content quality, not just delivery.

**Where it fires — the KAN-14 detection point, unchanged.** `ProgressUpdater.apply`, per active journey, already captures `old`/`new`, clamps + auto-completes (setting `completedAt`), and records a `WaypointCrossing` for every waypoint in the half-open interval `(old, new]` with `distanceFromStart > 0`. Phase 1 adds nothing to that detection: after the delta is applied AND `context.save()` succeeds, `apply` builds `[MilestoneNotificationRequest]` from the waypoints crossed *in this delta* plus the completion flag, and calls `NotificationManager.shared.enqueue(_:)` **once**. `enqueue` is the Phase-0 `nonisolated`, non-blocking primitive — no `await` ever enters the delta transaction.

### Ten rulings (KAN-33, Jake)

1. **Invalid `MilestoneNotificationRequest` states become unrepresentable (resolves the KAN-32 Rooster note).** The `hook: Hook` + `waypointID: UUID?` pair currently makes two invalid states representable (`.waypointReached` with nil id, `.journeyComplete` with a stray id). Replace them with a sum type that carries the waypoint only where it exists:
   ```swift
   ...
   ```
   The request/thread/category identifiers, the `hook` string, and the `userInfo` derive from this with no nil-fallback branch and no "complete-with-waypoint" leak. Phase 1 is `MilestoneNotificationRequest`'s first real consumer, so tightening the initializer now is free. *Rationale: make illegal states unrepresentable rather than assert around them.*

2. **Per journey, per delta: AT MOST ONE notification (the batching + completion-collision rule).** One delta can cross several waypoints and/or complete a journey; iOS delivers each `add` as its own banner, so N requests are the "burst" the ticket forbids — the per-run thread id only groups them *after* arrival. So Phase 1 **collapses**, per journey per `apply`: if the journey **completed** in this delta → fire `journey_complete` only, suppressing every waypoint banner (including the terminal waypoint's crossing); else if ≥1 non-origin waypoint was crossed → fire **one** `waypoint_reached` for the **furthest** (highest `distanceFromStart`) crossed; else fire nothing. Intermediate crossings are still recorded as `WaypointCrossing` data (the reached-log needs them) — they simply don't each notify. This also settles the terminal collision: every authored sheet already maps its terminal waypoint (at `distanceFromStart == totalDistance`) to the `journey_complete` hook, so completion is the single celebratory event and there is never a "reached the end" + "journey complete" pair from one reading. Simultaneous journeys each independently get their own at-most-one, keyed by run (per-run request + thread id).

3. **Idempotency = crossing existence + the monotonic anchor + deterministic identifiers — NO new field, NO model change.** Three guards already make a re-applied delta safe: the monotonic-anchor check (`newCumulative > lastProcessedDistance`) skips the whole crossing/notify block on a replay; `recordCrossings` skips any `waypointID` already recorded; and the deterministic `requestIdentifier` makes a re-add *replace* rather than duplicate. The notification decision is therefore driven off *this delta's* newly-inserted crossings + the completion-just-set flag, never "everything ever crossed." A persisted "notified" marker is unnecessary and is explicitly **not** added — crossing existence is the source of truth, mirroring KAN-14. *Consequence:* a milestone reached while unauthorized records its crossing but never notifies and (per Ruling 9) never replays later — forward-only, exactly like KAN-14/KAN-32 Ruling 3.

4. **Content lives in bundled CSV sheets read by a single `NotificationContentProvider`; parse + placeholder-fill happen there, numbers through `DistanceFormatter`/`StatFormatter`.** The sheets currently live only in `docs/notifications/*.csv` and are **not in the app target** (the target is a file-system-synchronized `JourneyTracker/` group; `docs/` sits outside it). Decision: **relocate the canonical CSVs into the target** at `JourneyTracker/Notifications/Content/*.csv`, where the synchronized group bundles them automatically as resources — one canonical copy, no build-time codegen, no docs↔bundle drift. (They stay CSV, git-tracked, and Numbers-editable; the `docs/notifications/README.md` path reference is updated by the coordinator as it is not Jake-owned.) A single **`NotificationContentProvider`** owns CSV parsing (quoted-field aware — bodies contain commas) and placeholder resolution; it is the one authority for notification copy, mirroring `DistanceFormatter`/`StatFormatter` as the one authority for numbers. Every `{miles_*}`/`{total_miles}` placeholder is filled via `DistanceFormatter` and every derived value (`{next_waypoint}`, `{miles_to_next}`, `{miles_remaining}`) via `JourneyStatsCalculator` at fire time — no literal mileage, no hand-formatting. **Journey→sheet lookup is name-keyed via a static catalog, mirroring `MapAuthoringCatalog`** (which keys map authoring by `JourneyTemplate.name`): the per-journey CSV slug is curated there (`"The Road to Ember Spire" → ember-spire`, etc. — the slug is not a pure derivation of the name), so no model/seed field is added. A journey with no sheet notifies nothing (graceful no-op), exactly as a journey with no map authoring falls back to pin-and-route. Waypoint rows are matched on `Waypoint.order` (the structural key the crossing snapshots).

5. **`{character}` resolves to a single constant ("Wren") from one seam.** There is no `Character` model or selection feature yet (future-proofing row: *Decided, not built*), and the app already hardcodes "Wren" in the KAN-7 completion banner. `{character}` reads from **one** `Character.currentName`-style constant seam (not scattered); when character selection ships, both the banner and notifications swap to the selected name in that one place. No new model, no seed change now.

6. **Requests are built inside the actor from Sendable snapshots, after `save`; `enqueue` once.** `JourneyStatsCalculator.stats(for:)` and the content provider are pure and read model properties, so they run on the `ProgressStore` actor's own context during `apply` — but only pure, in-memory CPU (no `await`, no I/O). `apply` accumulates a `[MilestoneNotificationRequest]` (all Sendable UUIDs + resolved strings) across active journeys, and **after** `try context.save()` succeeds calls `enqueue` once. Building before-save-commit but firing only after-success keeps a notification from ever describing a milestone the store didn't persist. The actual `UNUserNotificationCenter.add` stays on Phase 0's detached task. The request-building itself is factored out of `ProgressUpdater` into a pure, testable `MilestoneNotificationFactory` (crossed-waypoint snapshots + completion flag + resolved content → requests) so the actor file stays focused.

7. **`willPresent` foreground policy: quiet, not hard-suppressed.** Real milestones arrive backgrounded (iOS presents by default), so `willPresent` only fires when a delta lands while the app is foreground-active. Ruling: present **`[.banner, .list]` without `.sound`** — a glanceable, silent banner that still lands in Notification Center. This confirms the KAN-32 lean toward quieting (the in-app UI already reflects progress) without the coupling of true on-map suppression: detecting "is *this* journey's map on screen" would tie the notification layer to navigation state, and a soundless banner over a live map is harmless. A future `NotificationManager.foregroundJourneyID` set by the map on appear/disappear could add hard on-map suppression additively; not built now. Any in-app *visual* treatment is Jeff's, not this doc's.

8. **`didReceive` deep-link: `userJourneyID` → `UserJourney`, fallback `templateID`; opens that journey's `JourneyMapView`.** The tap handler resolves `userJourneyID` (UUID) to a `UserJourney` by fetching on `id`; if the instance was deleted, it falls back to `templateID` → the highest-precedence instance (active > paused > completed) of that template; if none resolves, it lands on the Journeys tab root (graceful, never a crash). Routing needs a seam the current `NavigationLink`-only stack lacks: a small `@MainActor @Observable` router holding a pending deep-link that `RootView` observes — it selects the Journeys tab and drives a **path-based** `NavigationStack` (`JourneyListView` moves to `NavigationStack(path:)` with a `navigationDestination` for the resolved instance) to push `JourneyMapView(journey:)`. The delegate is owned by `NotificationManager` (the only type touching `UNUserNotificationCenter`) via a private `NSObject` conformer, and **assigned at launch** (alongside `registerCategories()`) so a cold-launch tap's `didReceive` is caught. **`waypointID` is carried in the payload but scroll-to-waypoint is deferred**: the in-tab journey view is a static, chapter-framed map with no per-waypoint scroll API; framing the full-screen map's camera on a deep-linked waypoint is a later enhancement, out of scope here.
...
| **Notifications & milestones** | Built (KAN-6: fields only); crossing timing added (KAN-14); Phase 0 foundation Decided (KAN-32); Phase 1 milestones Decided (KAN-33) | Notify when passing a named landmark | Waypoints as bare coordinates with no metadata; scattering `UNUserNotificationCenter` calls across the app | Give each waypoint a `name` and `description` now, even if unused today — costs nothing, enables notifications later without a model change. KAN-14 adds `WaypointCrossing` records (when a run crossed each waypoint), written by `ProgressUpdater` — the same detection point Phase 1's crossing notification fires from. KAN-32 (epic KAN-26) adds the single `NotificationManager` authority, a `nonisolated` non-blocking enqueue callable from the `ProgressStore` actor, a UUID-based deep-link `userInfo` schema, and category/thread/request-identifier scheme — no model change. KAN-33 (Phase 1) fires the real waypoint-reached / journey-complete notifications from `ProgressUpdater.apply` after save (one per journey per delta, furthest-crossed or completion), with a `NotificationContentProvider` (bundled CSV sheets + placeholder fill via `DistanceFormatter`/`StatFormatter`) and the `UNUserNotificationCenterDelegate` (quiet foreground presentation + tap-to-map deep link) — still no model change. See "Notifications v1 — Phase 0" and "— Phase 1". |
